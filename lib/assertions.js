...
function getArrayFunctions(oBody){
    ...
    return {
        ...
        length(v){
            assert.strictEqual(oBody.length, v);
            return this;
        }
    }
}
...
module.exports = function(data){
    ...
    return {
        status: {
            ...
            type: expected => assert.strictEqual(String(data.status).charAt(0), String(expected)),
            notType: unwanted => assert(String(data.status).charAt(0) != String(unwanted))
        },
